...
const newCharacter = async (req, res, next) => {
  try {
    ...
    if (img && name && age && weight && history) {
      ...
      res.status(410).send({
        Error: 'Por Favor rellene todos los campos para crear el Personaje',
      });
    }
  } catch (err) {
    next(err);
    ...
    if (id) {
      ...
      if (!searchIdCharacter) {
        return res.status(409).send({
          Error: 'Por favor ingrese un ID de un Personaje valido',
        });
      } else {
        ...
        return res.status(200).send('Personaje eliminado', deleteChar);
      }
    } else {
...
const editCharacter = async (req, res, next) => {
  try {
    let { id, img, name, age, weight, history } = req.body;

    let searchId = await Character.findByPk(id);

    if (!searchId) {
      // validamos si nos trae un arreglo vacio
